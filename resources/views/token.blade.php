...
<head>
    ...
    <title>NarancoBioExplorer - Generar tokens</title>
...
<script>
    document.getElementById('crear-token').addEventListener('click', function() {
    fetch('/crear-token')
        .then(response => response.json())
        .then(data => {
            if (data.error) {
                document.getElementById('ver-token').innerHTML = `<p style='color:red'><strong>${data.message}</strong></p>`;
            } else {
                document.getElementById('ver-token').innerHTML = `<p><strong>Token (${data.rol}): ${data.token}</strong></p>`;
            }
        })
